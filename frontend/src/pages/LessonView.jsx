...
const LessonView = ({ user, config, setConfig }) => {
  ...
  useEffect(() => {
    const handleResearchWithdrawal = () => {
      isWithdrawingRef.current = true;
    };

    window.addEventListener(
      'smartstudy-research-withdrawal',
      handleResearchWithdrawal
    );
  ...
  useEffect(() => {
    ...
    axios.post(`${API_URL}/api/update-learning-session`, {
      userId: user?.userId,
      lesson_id: id,
    ...
    return () => {
      ...
      const duration = Math.floor(
        (Date.now() - lessonOpenedAtRef.current) / 1000
      );
      ...
      axios.post(`${API_URL}/api/update-learning-session`, {
        ...
        console.error(
          'Learning session final update failed:',
          error
        );
      });
    };
  ...
  useEffect(() => {
    if (!content || showQuiz || hasReachedEnd || !readingEndRef.current) return;
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const fastScrollCandidate =
        scrollSpeed > 1.1 &&
        percent > 35 &&
        !manualShowMindMap;

      const fastScrolling = fastScrollCandidate;
  ...
  const renderTextBlock = (text, className = '', isAlreadyHighlighted = false) => (
    <div
      className={`border-l-4 border-l-[#0a86dd] ${className}`}
    >
      <p
  ...
  const renderBottomMindMap = () => {
    ...
    return (
      <div ref={mindMapSectionRef} className="mt-2">
        <MindMapCard
          t={t}
          currentMindMap={currentMindMap}
          mindMapHighlight={mindMapHighlight}
