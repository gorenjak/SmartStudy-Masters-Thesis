...
def assign_variant():
    static_count = db.users.count_documents({"variant": "static"})
    adaptive_count = db.users.count_documents({"variant": "adaptive"})

    if static_count > adaptive_count:
        return "adaptive"

    if adaptive_count > static_count:
        return "static"

    return random.choice(["static", "adaptive"])
...
def get_collaborative_score(
    ...
    co_occurrence_count = db.learning_sessions.count_documents({
...
def set_user_style():
    ...
    try:
        ...
        valid_age_groups = ['<20', '20-24', '25-29', '30+']
